...
impl MemoryStorage {
    ...
    pub fn upsert_device(&self, mut device: DeviceInfo) -> Result<()> {
        let mut inner = self.inner.write().map_err(|_| anyhow::anyhow!("Failed to acquire write lock"))?;
    ...
    pub fn get_all_devices(&self) -> Result<Vec<DeviceInfo>> {
        let inner = self.inner.read().map_err(|_| anyhow::anyhow!("Failed to acquire read lock"))?;
    ...
    pub fn get_device_stats_history(&self, device_id: &str, limit: Option<usize>) -> Result<Vec<DeviceStats>> {
        let inner = self.inner.read().map_err(|_| anyhow::anyhow!("Failed to acquire read lock"))?;
    ...
    pub fn get_storage_info(&self) -> Result<StorageInfo> {
        ...
        let total_stats_entries = inner.stats_history.values()
            .map(|v| v.len())
            .sum();
...
mod tests {
    use super::*;
    use crate::api::models::{DeviceType, DeviceStatus};
