...
impl ServerHandler for AxectlMcpServer {
    ...
    async fn call_tool(
        ...
        // Get arguments or use empty object if none provided
        let args = request.arguments.unwrap_or_default();
        let args_value = serde_json::Value::Object(args);

        // Match tool name and dispatch to appropriate method
        match request.name.as_ref() {
            "discover_devices" => {
                let params: DiscoverDevicesRequest = serde_json::from_value(args_value.clone())
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.discover_devices(params).await)
            }
            "list_devices" => {
                let params: ListDevicesRequest = serde_json::from_value(args_value.clone())
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.list_devices(params).await)
            }
            "get_device_stats" => {
                let params: GetDeviceStatsRequest = serde_json::from_value(args_value.clone())
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.get_device_stats(params).await)
            }
            "get_device_config" => {
                let params: GetDeviceConfigRequest = serde_json::from_value(args_value.clone())
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.get_device_config(params).await)
            }
            "set_fan_speed" => {
                let params: SetFanSpeedRequest = serde_json::from_value(args_value.clone())
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.set_fan_speed(params).await)
            }
            "restart_device" => {
                let params: RestartDeviceRequest = serde_json::from_value(args_value.clone())
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.restart_device(params).await)
            }
            "update_settings" => {
                let params: UpdateSettingsRequest = serde_json::from_value(args_value.clone())
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.update_settings(params).await)
            }
            "wifi_scan" => {
                let params: WifiScanRequest =
                    serde_json::from_value(args_value.clone()).map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.wifi_scan(params).await)
            }
            "bulk_restart" => {
                let params: BulkRestartRequest = serde_json::from_value(args_value.clone())
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.bulk_restart(params).await)
            }
            "bulk_set_fan_speed" => {
                let params: BulkSetFanSpeedRequest = serde_json::from_value(args_value.clone())
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.bulk_set_fan_speed(params).await)
            }
            "bulk_update_bitcoin_address" => {
                let params: BulkUpdateBitcoinAddressRequest = serde_json::from_value(args_value)
                    .map_err(|e| {
                        McpError::invalid_params(format!("Invalid parameters: {}", e), None)
                    })?;
                Ok(self.bulk_update_bitcoin_address(params).await)
            }
