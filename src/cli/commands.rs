...
impl Cli {
    pub async fn run(self) -> Result<()> {
        ...
        match self.command {
            ...
            Commands::List { all } => handlers::list(all, self.format, !self.no_color).await,
            Commands::Stats {
                device,
                watch,
                interval,
...
pub mod handlers {
    ...
    pub async fn list(all: bool, format: OutputFormat, color: bool) -> Result<()> {
        use crate::api::DeviceStatus;
        use crate::output::{format_table, print_info, print_json};
                    ...
                    if all_stats.is_empty() {
                        ...
                        if all_stats.len() > 1 {
                            let total_hashrate: f64 =
                                all_stats.iter().map(|s| s.hashrate_mhs).sum();
    ...
    ) -> Result<crate::api::DeviceStats> {
        use std::time::Duration;
        let client =
            crate::api::AxeOsClient::with_timeout(&device.ip_address, Duration::from_secs(5))?;
    ...
    pub async fn monitor(
        ...
        print_info(
            &format!("Starting continuous monitoring ({}s interval)", interval),
            color,
        );
                    ...
                    Ok(stats) => {
                        ...
                        if let Some(hashrate_threshold) = hashrate_alert {
                            if let Some(previous_hashrate) =
                                previous_hashrates.get(&stats.device_id)
                            {
